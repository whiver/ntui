...
Credits = View:extend
{
	...
	onNew = function (self)
		...
		self.title3 = CreditsTitle:extend{y = self.text2.y + height + self.lineSpace, text = lang.creditsJoris}
		self:add(self.title3:new{})
		height = ({self.title3:getSize()}) [2]
		self.text3 = CreditsText:extend{y = self.title3.y + height + self.titleSpace, text = "Joris Chavot"}
		...
		self:add(bsod)
		self:add(glitch)
		effectOut = function () effect.visible = false end
		effectIn = function () effect.visible = true end
		effectPlayer = function()
			effectIn()
			self.timer:after(math.random()/3, effectOut)
			self.timer:after(math.random()*5+1, effectPlayer)

		end

		self.timer:after(math.random()*5, effectPlayer)
		self.timer:every(2, function() if math.random(2) == 2 then effect = glitch else effect = bsod end end)
